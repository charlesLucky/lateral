...
def generateDataset(ds_remark, byIDorByImages=True, train_weight=0.5, train_dir_tent='data/tmp_tent/train/',
                    includeST=True, includeTentnAquaBoth=False):
    ...
    SPLIT_WEIGHTS_INTRA_ID = (
        ...
        0.0)  # train cv val vs test for each identity, 50% are taken as train and 50% as test
    ...
    if byIDorByImages == 1:
        ...
        generateDatasetBySplitingIdentity('data/' + ds_remark + '/SESSION1_LT', train_ID_list, train_dir_tent,
                                          test_dir_tent, pre='sess1')
        generateDatasetBySplitingIdentity('data/' + ds_remark + '/SESSION2', train_ID_list, train_dir_tent,
                                          test_dir_tent, pre='sess2')
        generateDatasetBySplitingIdentity('data/' + ds_remark + '/SESSION3', train_ID_list, train_dir_tent,
                                          test_dir_tent, pre='sess3')
        generateDatasetBySplitingIdentity('data/' + ds_remark + '/SESSION4', train_ID_list, train_dir_tent,
                                          test_dir_tent, pre='sess4')
...
def aug_data_sess1(orig_path, SAVE_PATH, k=1,
                   num_aug_per_img=20):  # use k images from testing dataset as gallery and train the model into a classfication model for this ten IDs
...
def loadTestDS(test_data_dir, BATCH_SIZE=64, cfg=None):
    def get_label(file_path):
        parts = tf.strings.split(file_path, '/')
        label = tf.strings.to_number(parts[-2], out_type=tf.dtypes.int64)
        return label
...
def loadTestDS_shift(test_data_dir, BATCH_SIZE=64, cfg=None,shiffting_num=100):
    ...
    def _transform_images(is_ccrop=False, cfg=None):
        def transform_images(x_train):
            x_train = tf.image.resize(x_train, (cfg['input_size_w']+30, cfg['input_size_h']+30))
            x_train = tf.image.random_crop(x_train, (cfg['input_size_w'], cfg['input_size_h'], 3))
            x_train = tf.image.random_flip_left_right(x_train)
            x_train = tf.image.random_saturation(x_train, 0.6, 1.4)
            x_train = tf.image.random_brightness(x_train, 0.4)
